...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        Table newTbl = null;
        if (!tbl2Exist) {
            newTbl = tbl.select(clmNames, conds);
        } else {
            newTbl = tbl.select(tbl2, clmNames, conds);
        }
    ...
    Condition condition(Table... tables) {
        ...
        Condition retValue = null;

        int sze = tables.length;

        if (_input.nextIs(Tokenizer.LITERAL)) {
            String lit = literal();
            if (sze == 1) {
                retValue = new Condition(new Column(colName, tables[0]),
                    rel, lit);
            } else {
                retValue = new Condition(new Column(colName, tables[0],
                    tables[1]), rel, lit);
            }
        } else if (_input.nextIs(Tokenizer.IDENTIFIER)) {
            String colName2 = columnName();
            if (sze == 1) {
                retValue = new Condition(new Column(colName, tables[0]),
                    rel, new Column(colName2, tables[0]));
            } else {

                retValue = new Condition(new Column(colName, tables[0],
                    tables[1]), rel,
                    new Column(colName2, tables[1], tables[0]));
            }
        }
        return retValue;
    }
