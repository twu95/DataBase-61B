...
class Condition {
    ...
    boolean test(Row... rows) {
        int x;
        if (_col2 == null) {
            x = _col1.getFrom(rows).compareTo(_val2);
        } else {
            String row1Var = _col1.getFrom(rows);
            String row2Var = _col2.getFrom(rows);
            x = row1Var.compareTo(row2Var);
        }
    ...
    /** Relation like ">" or something like taht. */
